...
if st.button("Calculate New Tuition"):
    try:
        if sum(num_students) == 0 or len(current_tuition) == 0:
            ...
            # Summary of Calculation Steps (Formatted for clarity)
            summary_text = f"""
            ### Summary of Calculations:
            1. **Operations Tuition Increase (OTI)**:
               - Rate of Inflation (ROI): {roi_percentage:.2f}%
               - Rate of Productivity Increase (RPI): {rpi_percentage:.2f}%
               - Total OTI: {oti:.2f}%
            
            2. **Strategic Items**:
               - Strategic Items Cost Per Student: {cost_per_student:.2f}
               - Strategic Items (SI) Percentage: {si_percentage:.2f}%
            
            3. **Final Tuition Increase**:
               - OTI: {oti:.2f}%
               - SI: {si_percentage:.2f}%
               - Total Increase: {final_tuition_increase:.2f}%
            
            4. **Tuition Assistance**:
               - Total Financial Aid: {format_currency(financial_aid)}
               - Tuition Assistance Ratio: {tuition_assistance_ratio:.2f}%
            """
