...
report_title = st.text_input("Step 1: Enter a Custom Title for the Report", "2025-26 Tuition Projection")

# Step 2: Add Custom Grade Levels and Tuition Rates
with st.expander("Step 2: Add Custom Grade Levels and Tuition Rates", expanded=True):
    ...
    num_grades = st.number_input(
        "Number of Grade Levels", min_value=1, max_value=12, value=1, step=1
    )
    ...
    for i in range(num_grades):
        ...
        grades_data["num_students"][i] = st.number_input(
            f"Number of Students in {grades_data['grades'][i]}", min_value=0, value=grades_data["num_students"][i]
        )
...
with st.expander("Step 3: Automatically Calculated Average Tuition"):
...
with st.expander("Step 5: Enter Previous Year's Total Expenses"):
...
with st.expander("Step 6: Operations Tuition Increase (OTI) and Final Increase Calculation"):
    ...
    st.write(f"**Operations Tuition Increase (OTI):** {oti:.2f}%")
    st.write(f"**Strategic Items Increase:** {si_percentage:.2f}%")
    st.write(f"**Final Tuition Increase:** {final_tuition_increase:.2f}%")

# Step 7: Financial Aid (Tuition Assistance) Calculation
with st.expander("Step 7: Financial Aid (Tuition Assistance)"):
...
if st.button("Calculate New Tuition"):
    ...
    if "adjusted_tuition" not in st.session_state:
        st.session_state.adjusted_tuition = [tuition * (1 + final_tuition_increase / 100) for tuition in grades_data["current_tuition"]]
    ...
    st.session_state['calculation_results'] = {
        ...
        'summary_text': "Updated summary of tuition adjustment calculations based on user inputs.",
    }
    ...
    with st.container():
        tuition_data = {
            ...
            "Adjusted New Tuition per Student": [format_currency(tuition) for tuition in st.session_state.adjusted_tuition],
        }
        ...
        edited_df = st.data_editor(
            ...
            disabled=["Grade", "Number of Students", "Current Tuition per Student"],
            key="tuition_table"
        )
        ...
        if edited_df is not None:
            for i, row in edited_df.iterrows():
                if row["Adjusted New Tuition per Student"] != format_currency(st.session_state.adjusted_tuition[i]):
                    # Convert formatted currency back to float
                    new_value = float(row["Adjusted New Tuition per Student"].replace("$", "").replace(",", ""))
                    st.session_state.adjusted_tuition[i] = new_value

        # Recalculate totals based on adjusted tuition
        ...
        if 'calculation_results' in st.session_state:
            pdf_buffer = generate_pdf(
                ...
                st.session_state['calculation_results']['strategic_items_df'],
                st.session_state['calculation_results']['summary_text'],
            )
